...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check rally timeline regression targets against rally timeline JSON files.")
    parser.add_argument(
        "--suite-file",
    ...
    only = set(args.only)
    timeline_overrides = _suite_override_map(args.timeline)
    ...
    for suite_spec in suite_specs:
        ...
        default_timeline_path = suite_spec["timeline_json"]
        ...
        result = compare_timeline_suite(list(timeline.get("points", [])), suite_spec)
        print(
            f"[{suite_name}] "
            f"ok={result['ok']} "
